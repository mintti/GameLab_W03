    ...
    IEnumerator Battle()
    {
        ...
        while (currentPlayerHP > 0 && currentMonsterHP > 0)
        {
            if (knightTurn)
            {
                ...
                _uiManager.OutputCombatText("용사", _monster.Name, _knight.Status.Power, currentMonsterHP);
                knightTurn = false;
                monsterTurn = true;
            }
            else if (monsterTurn)
            {
                ...
                _uiManager.OutputCombatText(_monster.Name, "용사", _monster.Status.Power, currentPlayerHP);
                monsterTurn = false;
                knightTurn = true;
            }
